...
object FlinkEngine {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kafkaBrokers = AppConfig.getString(
      "kafka.brokers",
      "master:9092,slave1:9092,slave2:9092"
    )
    val ckUrl =
      AppConfig.getString("clickhouse.url", "jdbc:clickhouse://master:8123/ldc")
    ...
    // 模块 2.1 实时数据接入
    // ==========================================
    val sensorConsumer = new FlinkKafkaConsumer[String](
      "sensor_raw",
    ...
    val logConsumer = new FlinkKafkaConsumer[String](
      "log_raw",
      new SimpleStringSchema(),
      kafkaProps
    )
    ...
    val changeConsumer = new FlinkKafkaConsumer[String](
      "ChangeRecord",
      new SimpleStringSchema(),
      kafkaProps
    )
    ...
    // 2. 关联 Redis 计算状态和健康分
    val enrichedSensorStream =
      sensorStream.map(new RichMapFunction[SensorRaw, RealtimeStatus] {
        var jedis: Jedis = _
        override def open(parameters: Configuration): Unit = {
          jedis = new Jedis(redisHost, redisPort)
        }
        override def map(sensor: SensorRaw): RealtimeStatus = {
          val recordStatus = Option(
            jedis.hget("change_record_status", sensor.machine_id)
          ).getOrElse("")

          val machineStatus =
            if (recordStatus == "预警") "异常停机"
          ...
          val doUpdate =
            if (lastStr == null) true else (nowSec - lastStr.toLong) >= 60
          if (doUpdate) {
          ...
          new ProcessWindowFunction[
            ...
            override def process(
              ...
              if (count > 0) {
                ...
                val currentFluctuation = math
                  .sqrt((sumCurrentSq / count) - (meanCurrent * meanCurrent))
    ...
    val interpolated1m = window1m
      ...
      .process(new KeyedProcessFunction[String, WindowMetrics, WindowMetrics] {
        ...
        override def open(parameters: Configuration): Unit = {
          lastMetricsState = getRuntimeContext.getState(
            new ValueStateDescriptor("lastMetrics", classOf[WindowMetrics])
          )
          prevVibRmsState = getRuntimeContext
            .getState(new ValueStateDescriptor("prevVibRms", classOf[Double]))
          jedis = new Jedis(redisHost, redisPort)
        }
        ...
        override def processElement(
          ...
          val finalMetrics = if (lastMetrics != null) {
            val gapSec =
              (value.window_end.getTime - lastMetrics.window_end.getTime) / 1000
          ...
          if (prevVib > 0.0) {
            ...
            if (pct >= AppConfig.getDouble("vibration.spike.percent", 0.2)) {
              val alert = AlertRecord(
                finalMetrics.window_end,
                finalMetrics.machine_id,
                f"振动突增:${pct * 100}%.2f%%%%",
                finalMetrics.avg_vib_rms,
                0.0,
                "请立即检查设备",
              ...
              // 写入 Kafka
              val alertJson =
                "{\"machine_id\":\"" + alert.machine_id + "\",\"alert_time\":\"" + alert.alert_time + "\",\"alert_type\":\"" + alert.alert_type + "\",\"trigger_value\":" + alert.trigger_value + ",\"threshold_value\":" + alert.threshold_value + ",\"suggested_action\":\"" + alert.suggested_action + "\"}"
              jedis.rpush(
                "temp_kafka_alerts",
                alertJson
              ) // 使用 Redis 作为中转，或者直接发 Kafka
            }
          }
    ...
    val sensorAlerts = sensorStream.flatMap { s =>
      if (s.temperature > tempThreshold || s.vibration_x > vibThreshold) {
        ...
        val triggerVal =
          if (s.temperature > tempThreshold) s.temperature else s.vibration_x
        val threshVal =
          if (s.temperature > tempThreshold) tempThreshold else vibThreshold
        Some(
          AlertRecord(
            s.timestamp,
            s.machine_id,
    ...
    allAlerts
      .map(a =>
        "{\"machine_id\":\"" + a.machine_id + "\",\"alert_time\":\"" + a.alert_time + "\",\"alert_type\":\"" + a.alert_type + "\",\"trigger_value\":" + a.trigger_value + ",\"threshold_value\":" + a.threshold_value + ",\"suggested_action\":\"" + a.suggested_action + "\"}"
      )
    ...
    // 任务 18: 实时工艺参数偏离监控
    // ==========================================
    val deviationStream = sensorStream
      .map(
        new RichMapFunction[
          SensorRaw,
          (String, Double, Double, Timestamp, Double, Double)
        ] {
          var jedis: Jedis = _
          override def open(parameters: Configuration): Unit = {
            jedis = new Jedis(redisHost, redisPort)
          ...
          override def map(
            ...
            val stdCurrentStr =
              jedis.hget("std_params:" + s.machine_id, "current")
            val stdSpeedStr = jedis.hget("std_params:" + s.machine_id, "speed")
            val stdCurrent =
      ...
      .filter(x =>
        x._2 > AppConfig.getDouble(
          "deviation.threshold",
          0.028
        ) || x._3 > AppConfig.getDouble("deviation.threshold", 0.028)
      )
      ...
      .process(
        new ProcessWindowFunction[
          ...
          override def process(
              ...
              out: Collector[ProcessDeviation]
          ): Unit = {
    ...
    tableEnv.executeSql("""
      CREATE TABLE sensor_detail_realtime (
        ...
        'connector' = 'hudi',
        'path' = 'hdfs://master:8020/hudi/dwd/sensor_detail_realtime',
    ...
    """)
    tableEnv.executeSql(
      "INSERT INTO sensor_detail_realtime SELECT machine_id, ts, temperature, vibration_x, vibration_y, vibration_z, `current`, noise, speed FROM sensor_raw"
    )

    val enrichedLogStream = logStream.map { l =>
      val severity =
        if (l.error_code == "999") "Critical"
        else if (l.error_code == "500") "High"
        else "Normal"
    ...
    tableEnv.executeSql("""
      CREATE TABLE device_log_realtime (
        ...
        'connector' = 'hudi',
        'path' = 'hdfs://master:8020/hudi/dwd/device_log_realtime',
        'table.type' = 'MERGE_ON_READ',
        'compaction.async.enabled' = 'true',
        'compaction.delta_commits' = '5'
      )
    """)
    tableEnv.executeSql(
      "INSERT INTO device_log_realtime SELECT _1 as machine_id, _2 as ts, _3 as error_code, _4 as error_msg, _5 as stack_trace, _6 as severity FROM log_raw"
    )
