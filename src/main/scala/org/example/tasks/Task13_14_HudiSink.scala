...
/**
 * 任务组 4：实时数据入湖 (Flink/Spark -> Hudi)
 * 13. 实时传感器流写入 Hudi (COW 普通表)
 * 14. 实时日志流写入 Hudi (COW 普通表)
 */
object Task13_14_HudiSink {

  def startHudiSinks(sensorRawDF: DataFrame, logRawDF: DataFrame): Unit = {
    // 任务 13: 实时传感器流写入 Hudi
    // 需求更改：不用 MOR 表，用正常表 COPY_ON_WRITE (COW)
    val hudiSensorOptions = Map(
      ...
      "hoodie.datasource.write.table.type" -> "COPY_ON_WRITE", // 使用正常表 COW
