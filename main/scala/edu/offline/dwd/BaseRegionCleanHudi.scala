...
object BaseRegionCleanHudi {
  def main(args: Array[String]): Unit = {
    ...
    val ss: SparkSession = SparkSession.builder()
      ...
      .appName("Hudi clean baseRegion")
      .enableHiveSupport()
      .getOrCreate()

    //    import ss.implicits._
    //2、从ods中读取base_region表的数据:过滤分区、修改分区字段，添加列、处理时间戳字段
    val odsUserInfo=ss.read.format("hudi")
      .load("hdfs://master:8020/user/hive/warehouse/ods_ds_hudi.db/base_region")
    ...
    odsUserInfo.write
      ...
      .option(TBL_NAME.key(),"dim_region") //表名
      .save("hdfs://master:8020/user/hive/warehouse/dwd_ds_hudi.db/dim_region") //文件路径
