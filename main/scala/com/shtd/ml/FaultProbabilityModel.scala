...
object FaultProbabilityModel {
  def main(args: Array[String]): Unit = {
    ...
    val featureDF = spark.read
      .format("hudi")
      .load(
        s"$hdfsUrl/user/hive/warehouse/dwd_ds_hudi.db/dwd_environment_features"
      )
    ...
    if (positiveRatio < 0.01) {
      ...
      var labeledDF = featureDF
        .withColumn(
          "label",
          when(
            $"avg_temp" > newTempThreshold || $"fft_peak" > newFftThreshold,
            1.0
          )
            .otherwise(0.0)
        )
      ...
      val newPositiveRatio =
        labeledDF.filter($"label" === 1.0).count().toDouble / labeledDF.count()
      if (newPositiveRatio < 0.01) {
        ...
        labeledDF = featureDF
          .withColumn(
            "label",
            when(
              $"avg_temp" > finalTempThreshold || $"fft_peak" > finalFftThreshold,
              1.0
            )
              .otherwise(0.0)
          )
    ...
    // 分别从正负样本中按比例抽取
    val Array(posTrain, posTest) =
      positiveData.randomSplit(Array(0.8, 0.2), seed = 42)
    val Array(negTrain, negTest) =
      negativeData.randomSplit(Array(0.8, 0.2), seed = 42)

    // 合并
    val trainData = posTrain.union(negTrain)
    val testData = posTest.union(negTest)
    // 【新增】从训练集中再分出验证集，用于检测过拟合
    val Array(trainFinal, validationData) =
      trainData.randomSplit(Array(0.85, 0.15), seed = 42)
    ...
    val xgb = new XGBoostClassifier(
      Map(
        ...
        // 关键：添加处理不平衡数据的参数
        "scale_pos_weight" -> (labeledDF
          .filter($"label" === 0.0)
          .count()
          .toDouble /
          labeledDF.filter($"label" === 1.0).count()),
        "min_child_weight" -> 5,
        "subsample" -> 0.6,
    ...
    println(
      s">>> 正负样本比例：1:${labeledDF.filter($"label" === 0.0).count().toDouble / labeledDF.filter($"label" === 1.0).count()}"
    )
    ...
    val trainAccuracy = trainPredictions
      .filter($"prediction" === $"label")
      .count()
      .toDouble / trainFinal.count()
    println(s">>> 训练集准确率：$trainAccuracy")

    // 验证集表现
    val valPredictions = model.transform(validationData)
    val valAccuracy = valPredictions
      .filter($"prediction" === $"label")
      .count()
      .toDouble / validationData.count()
    ...
    predictions
      ...
      .foreach { row =>
        ...
        println(
          s"label=$label, prediction=$pred, probability=${probVector.toArray.mkString("[", ",", "]")}, size=${probVector.size}"
        )
      }
    ...
    // 计算混淆矩阵
    val tp =
      predictions.filter($"label" === 1.0 && $"prediction" === 1.0).count()
    val tn =
      predictions.filter($"label" === 0.0 && $"prediction" === 0.0).count()
    val fp =
      predictions.filter($"label" === 0.0 && $"prediction" === 1.0).count()
    val fn =
      predictions.filter($"label" === 1.0 && $"prediction" === 0.0).count()
    ...
    finalModel.write
      .overwrite()
      .save(s"$hdfsUrl/models/fault_probability_xgboost_v2")
    ...
    try {
      ...
      val dstPath = new org.apache.hadoop.fs.Path(
        s"/models/fault_probability_xgboost_v2.bin"
      )
      // 覆盖写入
      fs.copyFromLocalFile(false, true, srcPath, dstPath)
      println(
        s">>> 原生 C++ 格式故障模型已上传到 HDFS: /models/fault_probability_xgboost_v2.bin"
      )
    } catch {
