...
object orderDetailTopNToRedis {
  def main(args: Array[String]): Unit = {
    ...
    val itemSummary:DataStream[ItemSummary]=filter_ds2.map(record=>{
        val fields=record.split(",")
        val formatter=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
        val createTime=formatter.parse(fields(8).split("\"")(1)).getTime
        ItemSummary(fields(3).split("\"")(1),fields(6).split("\"")(1).toDouble,fields(7).split("\"")(1).toInt,createTime)
      })
    ...
    val itemConsumptionSummary= itemSummary
      ...
      .process(new ProcessAllWindowFunction[(String, Double),(String,String),TimeWindow] {
        def process(context: Context, elements: Iterable[(String, Double)],
          ...
          out.collect("top3itemconsumption", top3)
        }
      })
