...
object ExportDissolvedOxygenDataToDB {
  def main(args: Array[String]): Unit = {
    ...
    val sensor = "DO"
    val measurementTime = "2025-03-21 08:00:00"
    // 经度和纬度的新范围
    ...
    try {
      ...
      // 准备插入语句
      val insertSQL = """
        INSERT INTO dissolved_oxygen_data
          (stationID, stationName, longitude, latitude, measurementTime, sensor, DO)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      """
      preparedStatement = connection.prepareStatement(insertSQL)

      // 生成并插入1000条数据
      for (stationID <- 1 to 1000) {
        // 生成随机的longitude, latitude和DO值
        ...
        val doValue = 4.5 + random.nextDouble() * 5.5 // 溶解氧浓度范围：4.5 ~ 10.0 mg/L
        ...
        preparedStatement.setBigDecimal(7, new JavaBigDecimal(doValue).setScale(2, RoundingMode.HALF_UP))
