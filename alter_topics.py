...
try:
    ...
    for topic in topics:
        cmd = f"/opt/module/kafka/bin/kafka-topics.sh --alter --bootstrap-server 192.168.45.100:9092 --topic {topic} --partitions 3"
        out, err = run_cmd(ssh, cmd)
        print(f"Altered topic {topic}: {out} | {err}")
        
    for topic in ["device_state", "sensor_metrics"] + topics:
