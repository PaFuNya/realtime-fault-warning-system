error id: file:///D:/Desktop/Match/MATCH/OverMatch/src/main/scala/org/example/tasks/Task2_4_RealtimeOutput.scala:
file:///D:/Desktop/Match/MATCH/OverMatch/src/main/scala/org/example/tasks/Task2_4_RealtimeOutput.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -org/apache/spark/sql/functions/SparkUtils.getCkProperties.
	 -org/apache/spark/sql/functions/SparkUtils.getCkProperties#
	 -org/apache/spark/sql/functions/SparkUtils.getCkProperties().
	 -SparkUtils.getCkProperties.
	 -SparkUtils.getCkProperties#
	 -SparkUtils.getCkProperties().
	 -scala/Predef.SparkUtils.getCkProperties.
	 -scala/Predef.SparkUtils.getCkProperties#
	 -scala/Predef.SparkUtils.getCkProperties().
offset: 1664
uri: file:///D:/Desktop/Match/MATCH/OverMatch/src/main/scala/org/example/tasks/Task2_4_RealtimeOutput.scala
text:
```scala
package org.example.tasks

import org.apache.spark.sql.functions._
import redis.clients.jedis.Jedis

/** 模块二：实时数据处理引擎 2.4 实时结果输出 需求：状态快照：每分钟更新 Redis 中的设备最新健康分。 实时指标：写入 ClickHouse
  * device_realtime_status 表，供大屏展示。
  */
object Task2_4_RealtimeOutput {

  def main(args: Array[String]): Unit = {
    val spark = SparkUtils.getSparkSession("Task2_4_RealtimeOutput")

    // 直接调用工具类读取
    val sensorRawDF = SparkUtils
      .getSensorRawStream(spark)
      .withColumn("health_score", lit(100))
      .withColumn("status", lit("稳定运行"))

    sensorRawDF.writeStream
      .foreachBatch {
        (batchDF: org.apache.spark.sql.DataFrame, batchId: Long) =>
          // 1. 2.4 状态快照：更新 Redis 中的设备最新健康分和状态
          batchDF
            .select("machine_id", "health_score", "status")
            .rdd
            .foreachPartition { partition =>
              val jedis = new Jedis("127.0.0.1", 6379)
              partition.foreach { row =>
                val mid = row.getAs[String]("machine_id")
                jedis.hset(
                  s"device_status:$mid",
                  "health_score",
                  row.getAs[Int]("health_score").toString
                )
                jedis.hset(
                  s"device_status:$mid",
                  "status",
                  row.getAs[String]("status")
                )
              }
              jedis.close()
            }

          // 2. 2.4 实时指标：写入 ClickHouse device_realtime_status 表
          try {
            batchDF.write
              .mode("append")
              .jdbc(
                SparkUtils.ckUrl,
                "device_realtime_status",
                SparkUtils.getCkPropert@@ies()
              )
          } catch { case _: Exception => }
      }
      .option("checkpointLocation", "/tmp/checkpoints/redis_status_standalone")
      .start()

    spark.streams.awaitAnyTermination()
  }
}

```


#### Short summary: 

empty definition using pc, found symbol in pc: 